...
test('Sunday stays closed and same-day past slots are rejected in IST', () => {
  ...
  assert.equal(normalizeAppointmentTime('9:00 AM'), '9:00 AM')
  assert.equal(normalizeAppointmentTime('1:00 PM'), '1:00 PM')
  assert.equal(normalizeAppointmentTime('14:00'), '2:00 PM')
  assert.equal(normalizeAppointmentTime('2:30 PM'), '2:30 PM')
  assert.equal(normalizeAppointmentTime('दोपहर 3:30 बजे'), '3:30 PM')
  assert.equal(normalizeAppointmentTime('सुबह नौ बजे'), '9:00 AM')
  assert.equal(normalizeAppointmentTime('8:59 AM'), null)
  assert.equal(normalizeAppointmentTime('5:01 PM'), null)
  assert.equal(normalizeAppointmentTime('5:00 PM'), '5:00 PM')
})
