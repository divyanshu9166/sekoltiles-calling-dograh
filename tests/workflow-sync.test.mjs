...
test('sync publishes compact customer tools without replacing chosen models or telephony', async () => {
  ...
  try {
    ...
    assert.match(start.data.prompt, /without explicit appointment means catalogue/)
    assert.match(start.data.prompt, /if still unclear offer human transfer and wait for explicit yes/)
    ...
    assert.ok(start.data.prompt.length < 9_000, `prompt is ${start.data.prompt.length} chars`)
